...
function ItemCard(props) {
  ...
  const goToFoodPage = () => {
    props.navigation.navigate("Food Page",  { ...props.data, service: 'nightCanteenItems' })
  }
  return <>
...
const Yadav = ({ navigation }) => {
  ...
  useEffect(() => {
    async function getData() {
      let querySnapshot = await getDocs(collection(db, "nightCanteenItems"));
      let arr = []
        ...
        <View className="border bg-white px-2 py-2 border-1 border-gray-300 h-24 my-5 rounded-lg">
          <View className="flex-row">
            <View className="w-10/12">
              <Text className="text-2xl font-bold">The Night Canteen</Text>
              <Text className="text-sm my-1 text-gray-600">Near by science block</Text>
            </View>
